...
@property (nonatomic, strong) UIImageView *cYDoonImageView;
...
- (void)setEmptyType:(EYDEmptyType)emptyType
{
    ...
    self.cYDoonImageView.hidden = YES;
    ...
    if (img) {
        self.cYDoonImageView.image = img;
        self.cYDoonImageView.hidden = NO;
        if (bottomConstraint) [bottomConstraint deactivate];
        [self.cYDoonImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(lastTopView);
            make.centerX.equalTo(lastTopView);
            bottomConstraint = make.bottom.equalTo(self);
        }];
        lastTopView = self.cYDoonImageView;
    }
...
- (void)makeUI
{
    ...
    [self addSubview:self.cYDoonImageView];
...
- (UIButton *)tipBtn {
    if (!_tipBtn) {
        UIButton *resetTipBtn = [YDEmptyViewConfig resetTipBtn];
        if (resetTipBtn) {
            _tipBtn = resetTipBtn;
        } else {
        _tipBtn = [[UIButton alloc] init];
        // fixme 有backgroudImage 为啥要设置颜色？
//        [_tipBtn setBackgroundColor:[UIColor colorWithRGBHex:0xF24F4F]];
...
//        _tipBtn.layer.cornerRadius = 20.f;
        _tipBtn.layer.cornerRadius = [YDEmptyViewConfig tipBtnCornerRadius];
...
- (UIButton *)subTipBtn {
    if (!_subTipBtn) {
        UIButton *resetSubTipBtn = [YDEmptyViewConfig resetSubTipBtn];
        if (resetSubTipBtn) {
            _subTipBtn = resetSubTipBtn;
        } else {
...
- (UIImageView *)cYDoonImageView
{
    if(!_cYDoonImageView){
        _cYDoonImageView = [[UIImageView alloc] init];
        _cYDoonImageView.image = [YDEmptyViewConfig defaultEmptyImage];
        _cYDoonImageView.backgroundColor = [UIColor clearColor];
    }
    return _cYDoonImageView;
}
